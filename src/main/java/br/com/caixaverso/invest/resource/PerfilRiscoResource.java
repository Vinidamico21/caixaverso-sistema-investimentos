...
public class PerfilRiscoResource {
    ...
    @Inject
    CalcularPerfilRiscoUseCase calcularPerfilUseCase;
    ...
    @APIResponses({
            ...
            @APIResponse(
                    responseCode = "404",
                    description = "Nenhuma simulação encontrada para o cliente ou perfil de risco inexistente.",
                    content = @Content(schema = @Schema(implementation = ApiErrorDTO.class))
            ),
            @APIResponse(
                    responseCode = "500",
    ...
    public Response obterPerfil(
        ...
        PerfilRiscoResponseDTO dto = calcularPerfilUseCase.calcularPerfil(clienteId);
